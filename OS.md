...
##### Y86-64 程序
- Y86-64无法使用立即数
- 伪命令：指明将代码或者数据放在什么位置以及如何对齐

##### 逻辑门
- 逻辑门是数字电路的基本计算单元 逻辑门：HCL
- AND:&&
- OR:||
- NOT:!

##### 组合电路和HCL布尔表达式
- 多个逻辑门组合成一个网，构成计算块，就称之为组合电路
- 每个逻辑门的输入必须连接到，1）系统输入，2）某个存储单元的输出，3）某个逻辑门的输出
- 两个或者多个逻辑门的输出不能连在一起
- 这个组合电路不能是环
- HCL硬件控制语言，例子：bool eq = (a && b) || (!a && !b)

##### 字级的组合电路和HCL整数表达式
- 多个位级电路组成一个字级电路
  - 例如：判断两个二进制数值是否相等，则对每一位进行比较，每一次比较就是一份位级的组合电路
- 通过多路复用的思路来控制输入位的值来决定字级电路的输出

##### 存储器和时钟
- 对于任意的电路组件，当提供了输入信号之后总是要“等一会儿”，然后输出引脚上的信号才会变成正确的结果，这个等待的过程叫着同步
- 时钟周期的频率就是系统执行指令的频率
- 每当每个时钟到达上升沿时,值才会从寄存器的输入传送到输出
- 时钟上升沿指的是数字时钟电路中，数字电平从低电平（数字“0”）变为高电平（数字“1”）的那一瞬间（时刻）叫作上升沿
- 振荡器（Oscillator）：模拟时钟信号的电子元件

##### 将处理组织成阶段
- 取指 (fetch) : 取指阶段从内存读取指令字节,地址为程序计数器 (PC) 的值
- 译码 (decode): 译码阶段从寄存器文件读入最多两个操作数,得到值 valA 和/或 valB
- 执行 (execute): 在执行阶段,算术/逻辑单元 (ALU) 要么执行指令指明的操作(根据 ifun 的值),计算内存引用的有效地址,要么增加或减少栈指针
- 访存 (memory): 访存阶段可以将数据写入内存,或者从内存 读出数据
...
- **译码**:寄存器文件有两个读端口A和B,从这两个端口同时读寄存器值 valA和valB
...
- 白色方框表示时钟寄存器
- 浅蓝色方框表示硬件单元(ALU逻辑运算单元是一种组合数字电路，可对整数二进制数执行算术和按位运算)
- 控制逻辑块用灰色圆角矩形表示(HCL硬件控制语言，例子：bool eq = (a && b) || (!a && !b))
- 线路的名字在白色圆圈中说明
- 宽度为字长的数据连接用中等粗度的线表示
- 宽度为字节或更窄的数据连接用细线表示
- 单个位的连接用虚线来表示
